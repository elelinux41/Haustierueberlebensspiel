    ...
    def kaufen(self):
        ...
        try:
            ...
            if self.geld > (futter_anzahl * 15):
                ...
                if futter_anzahl > 7:
                    prüfen0 = input('Willst du wirklich ' + futter_anzahl + ' Portionen Futter kaufen(j/n): ')
                    prüfen0 = prüfen0.lower()
                    prüfen0 = prüfen0.strip()
                elif futter_anzahl <= 7 or prüfen0 == 'j' or prüfen0 == 'ja':
...
def modus1():
    global tier
    print()
    modus = input('Welche Spielmethode willst du nutzen, Standard(s) oder Benutzerdefiniert(b): ')
    modus = modus.lower()
    modus = modus.strip()
    if modus == 's' or modus == 'standard':
        ...
        def tier1():
            ...
            if tier0 == 'h' or tier0 == 'hund':
                if name == '':
                    name = random.choice(hundename)
                tier = TierHund(name,zeitraum)
            elif tier0 == 'k' or tier0 == 'katze':
                if name == '':
                    name = random.choice(katzenname)
                tier = TierKatze(name,zeitraum)
            elif tier0 == 'f' or tier0 == 'fisch' or tier0 == 'fische':
                if name == '':
                    name = random.choice(fischname)
                tier = TierFisch(name,zeitraum)
            else:
...
modus1()
print()
print('In diesem Spiel musst du dich ' + str(tier.zeitraum // 24) + ' Tage lang um das Haustier ' + tier.name +' deiner Nachbarn kümmern.')
